...
// models
import { ErrorService, RequestError } from '@vonbraunlabs/app-state';
...
export function testCatch(
	...
	spyOn(ErrorService, 'parseError').and.callThrough();

	target().subscribe(
		() => fail('Should throw an error'),
		err => expect(ErrorService.parseError).toHaveBeenCalledWith(err.raw)
	);
